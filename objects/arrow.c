...
void updateArrow(Arrow *arrow, double dt) {
  ...
  // Simple ground collision (stop if hits ground)
  // Ground is at y = -3.0 roughly, let's say -2.0 to be safe or just let it
  // fall User said "Dont worry about collision yet", so we let it fly forever
  // or until reset
}
...
void shootArrow(Arrow *arrow, double x, double y, double z, double th,
  ...
  // Converting view angles to direction vector:
  // th is azimuth, ph is elevation
  // If th=0 faces -Z, then:
  // z component should be negative when th=0.
  // x component should be 0 when th=0.
  // So:
  // dx = Sin(th) * Cos(ph)
  // dy = Sin(ph)
  // dz = -Cos(th) * Cos(ph)

  arrow->dx = Sin(th) * Cos(ph);
  arrow->dy = Sin(ph);
  arrow->dz = -Cos(th) * Cos(ph);
