...
void drawGround(double steepness, double size, double groundY,
  ...
  if (!groundList)
    needsRebuild = 1;
  if (cachedSteep != steepness || cachedSize != size || cachedY != groundY ||
      cachedTex != texture)
    needsRebuild = 1;
...
void drawMountainRing(double innerR, double outerR, double baseY,
  ...
  if (!ringList)
    rebuild = 1;
  if (cInner != innerR || cOuter != outerR || cBaseY != baseY ||
      cHeight != heightScale || cTex != texture)
    rebuild = 1;
